...
enum
{
    ...
    TRAP_YIELD = 0x26   /* yield processor for another process */
};
...
/* Write reg and memory to txt file*/
void dump_mem_to_txt(){

    string file_name = "memory_dump_";
    file_name += to_string(file_num);
    file_name += ".txt";

    ofstream mem_file;
    mem_file.open (file_name);
    for (int i = 0; i < R_COUNT; i++) {
        mem_file << "R" << to_string(i) << ": " << to_string(reg[i]) << "\n";
    }
